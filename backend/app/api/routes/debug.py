...
async def reset_demo():
    ...
    # Clear profile cache from Redis
    try:
        redis = await get_redis()
        await redis.delete(f"profile:{DEMO_USER_ID}")
        await redis.delete(f"cv_chunks:{DEMO_USER_ID}") # In case cv_service caches embeddings key
...
async def login_demo():
    ...
    async with get_db(DEMO_USER_ID) as db:
        ...
        row = result.first()
        if not row:
            # If demo user was somehow missing, seed it
